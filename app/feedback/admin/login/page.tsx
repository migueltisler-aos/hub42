...
async function login(formData: FormData) {
  ...
  const from = (formData.get("from") as string) || "/feedback/admin";

  if (!password) return;
  if (password !== process.env.FEEDBACK_PASSWORD) return;

  const cookieStore = await cookies();
  cookieStore.set("feedback_admin_auth", password, {
    path: "/",
    httpOnly: true,
    sameSite: "lax",
  });
...
export default async function FeedbackLoginPage({
  searchParams,
}: {
    ...
    <div className="min-h-screen bg-green-dark flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        ...
        <form action={login} className="space-y-4">
          <input type="hidden" name="from" value={from ?? "/feedback/admin"} />
