-- Go-live 24.09.2026: alle drei Migrationen in einem Durchlauf
-- Supabase → SQL Editor → New query → komplett einfügen → Run
...
-- Kontrolle: alle drei Zeilen müssen true zeigen
select
  to_regproc('public.analytics_track') is not null           as analytics_track,
  to_regclass('public.bewerbungen') is not null              as bewerbungen,
  exists (select 1 from information_schema.columns
          where table_name = 'pipeline_angebote' and column_name = 'besonderer_wert') as angebote_spalte;
